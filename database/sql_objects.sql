...
-- Триггерная функция для обновления статуса купленного абонемента
...
-- Привязываем триггер к правильной таблице 'client_subscriptions'
...
-- Привязываем триггер к таблице участников
...
-- Используем 'CASCADE' для автоматического удаления зависимых объектов (других представлений)
DROP VIEW IF EXISTS client_full_info_view CASCADE;
CREATE VIEW client_full_info_view AS
SELECT
    c.id as client_id, -- Явное именование для избежания конфликтов
    c.last_name,
    c.first_name,
    c.middle_name,
    c.reg_date,
...
SELECT
    s.id as staff_id, -- Явное именование
    s.last_name,
    s.first_name,
    s.middle_name,
    s.hire_date,
...
-- Удаляем старый индекс, если он существовал
